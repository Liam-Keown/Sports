...
class TeamPredictor(ScorePredictorRNN):
    def __init__(self, input_size, hidden_size, num_layers, team_embedding_size):
        super(TeamPredictor, self).__init__(input_size, hidden_size, num_layers)
        self.team_embedding = nn.Embedding(len(teams), team_embedding_size)  
        self.fc = nn.Linear(hidden_size + team_embedding_size, 2)  

    def forward(self, x, team_id):

        team_embedding = self.team_embedding(team_id)

        out = super().forward(x)

        out = torch.cat((out[:, -1, :], team_embedding), dim=1)
        out = self.fc(out)
        return out
...
base_input_size = sequences.shape[2]
base_hidden_size = 64
base_num_layers = 2
team_embedding_size = 16  

for team in teams.values:
    team_index = teams[teams == team].index[0]

    sequences_team, targets_team = create_sequences_for_team(desired_games, team, sequence_length)

    model = TeamPredictor(base_input_size, base_hidden_size, base_num_layers, team_embedding_size)
